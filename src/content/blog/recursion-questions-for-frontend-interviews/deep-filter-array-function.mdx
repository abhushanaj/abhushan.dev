...
seoMetaData:
  ...
  description: 'Implement a deepFilterArray function using recursion'
  ogImage: '/open-graph/blogs/implement-a-deep-flatten-array-function.webp'
  ogImageAlt: 'Implement a deepFilterArray function using recursion'
title: 'Implement a deepFilterArray function'
...
**Deep Flattening** is not a standard term defined in JavaScript, but it is commonly well understood as the process of perform filtering operation on each depth of a nested multi-dimensional array.
...
## 2. Counting filtered elements implementation
...
The implementation for the same remain almost the same, so I will highlight only the new changes made.
