...
seoMetaData:
  ...
  description: 'Implement a getElementsBy by using recursion.'
  ogImage: '/open-graph/blogs/get-elements-by-function.webp'
  ogImageAlt: 'Implement a getElementsBy by using recursion.'
title: 'Implement a getElementsBy function'
...
<SubHeading>Variation: By classNameHierarchy</SubHeading>
...
function getElementsByClassNameHierarchy(root, classHierarchy) {
	...
	function transverse(el, index = 0) {
		...
		if (elHasCurrentClassName) {
			// if it did have, we transverse child and look for second className
			for (const childEl of el.children) {
				transverse(childEl, index + 1);
			}
		} else {
			// if it did not have it, we transverse child and restart the search
			for (const childEl of el.children) {
				transverse(childEl, 0);
			}
		}
	}
