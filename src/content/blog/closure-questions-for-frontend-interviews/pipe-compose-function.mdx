...
**Pipe** and **Compose** are both higher order function inspired from functional programming languages to facilitate function composition and data transformation. They are exactly same in their behaviour with one minor difference i.e, the order in which ths transformation is doesNotMatch.
