...
seoMetaData:
  ...
  description: 'Implement an array iterator  by using Javascript closures.'
  ogImage: '/open-graph/blogs/implement-an-array-iterator-function.webp'
  ogImageAlt: 'Implement an array iterator  by using Javascript closures.'
...
1. <InlineCode code="done" /> : This is true if the last **value in the sequence has already been consumed**, else false.
...
However, for our use case, we have to implement a <InlineCode code="iteratorArray" /> method, which takes an array as argument and returns an obbject with <InlineCode code="next()" /> and <InlineCode code="done" /> properties. Calling the <InlineCode code="next()" /> method should return the next item in array until the end of array, after which it returns <InlineCode code="null" /> and <InlineCode code="done()" /> methods returns a boolean flag denoting if all iterations are completed.
